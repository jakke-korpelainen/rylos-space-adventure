...
const Rock = React.memo((props: IRockProps) => {
  ...
  useFrame(() => {
    try {
      if (clock) {
        const r = Math.cos((clock.getElapsedTime() / 2) * props.data.speed) * Math.PI
  ...
  return (
    <group ref={ref} position={props.data.offset} scale={[props.data.scale, props.data.scale, props.data.scale]}>
      <group
        ...
        scale={[20, 20, 20]}>
        <mesh geometry={props.nodes.node_id4_Material_52_0.geometry} material={props.materials.Material_52} material-roughness={1} material-metalness={1} />
      </group>
    </group>
  )
})
